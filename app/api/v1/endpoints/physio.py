...
@router.put(
    "/{date}",
    summary="Create or update physio entry for a date.",
    response_model=PhysioEntryResponse,
)
def upsert_physio(
    date: datetime.date,
    data: PhysioEntryCreate,
    response: Response,
...
@router.get(
    "",
    summary="List physio entries with optional filters.",
    response_model=list[PhysioEntryResponse],
)
def list_physio(
    date: Optional[datetime.date] = Query(None, description="Exact date filter"),
...
@router.get(
    "/{date}",
    summary="Get physio entry for a specific date.",
    response_model=PhysioEntryResponse,
)
def get_physio(
    date: datetime.date,
...
@router.delete(
    "/{date}",
    summary="Delete physio entry for a specific date.",
